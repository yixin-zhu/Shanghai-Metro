...
class Graph:
    ...
    def navigateBellmanFord(self, line):
        ...
        totalRoute = all[0]
        if(len(all) < 2):
            return "Please enter at least two stations!"
        for i in range(0, len(all)-1):
            self.bellmanFord(all[i])
            totalRoute = totalRoute + self.getRouteContinue(all[i+1])
            totalTime = totalTime + self.getTime(all[i+1])
            totalChangeTimes = totalChangeTimes + \
                self.getChangeTimes(all[i+1])
        totalRoute = totalRoute + "            Expected time:" + str(totalTime) + "minutes"
        totalRoute = totalRoute + "            Expected to change:" + str(totalChangeTimes) + "times"
        return totalRoute

    def navigateDijkstra(self, line):
        all = line.split()
        totalChangeTimes = 0
        totalTime = 0
        totalRoute = all[0]
